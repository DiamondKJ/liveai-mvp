require('dotenv').config();
const express = require('express');
const http = require('http');
const { Server } = require('socket.io');
const cors =require('cors');
const { nanoid } = require('nanoid');
const Anthropic = require('@anthropic-ai/sdk');

const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });

const app = express();
server = http.createServer(app);
const io = new Server(server, {
    cors: { origin: "http://localhost:5173" }
});

const rooms = new Map();

io.on('connection', (socket) => {
    console.log(`A user connected: ${socket.id}`);

    socket.on('create_room', (callback) => {
        const roomId = nanoid(8);
        rooms.set(roomId, {
            // NEW STATE FOR THE PROMPT CHAIN
            users: [{ id: socket.id, name: `User-${socket.id.substring(0,4)}` }], // An ordered list of users
            currentUserIndex: 0, // Whose turn is it? Starts with the host (index 0).
            promptInProgress: "", // The prompt being built.
            messages: [],
        });
        socket.join(roomId);
        console.log(`Room created: ${roomId} by host ${socket.id}`);
        callback({ roomId });
    });

    socket.on('join_room', ({ roomId }, callback) => {
        const room = rooms.get(roomId);
        if (room) {
            socket.join(roomId);
            // Add the new user to the end of the line
            if (!room.users.some(u => u.id === socket.id)) {
                 room.users.push({ id: socket.id, name: `User-${socket.id.substring(0,4)}` });
            }
            
            // Notify everyone of the new user list and whose turn it is
            io.to(roomId).emit('update_game_state', {
                users: room.users,
                currentUserIndex: room.currentUserIndex,
                promptInProgress: room.promptInProgress,
                messages: room.messages
            });
            callback({ status: "ok" });
        } else {
            callback({ status: "error", message: 'Room not found' });
        }
    });

    // COMPLETELY NEW LOGIC: This replaces 'send_message'
    socket.on('submit_contribution', async ({ roomId, updatedPrompt }) => { // The client will now send the *entire* updated prompt
        const room = rooms.get(roomId);
        if (!room) return;
    
        const currentUser = room.users[room.currentUserIndex];
        if (currentUser.id !== socket.id) {
            return; // Not your turn!
        }
    
        // --- THIS IS THE KEY CHANGE ---
        // Instead of adding, we REPLACE the prompt with the user's edited version.
        room.promptInProgress = updatedPrompt;
        // --- END OF KEY CHANGE ---
        
        // Advance the turn
        room.currentUserIndex++;
    
        // Check if the chain is complete (same logic as before)
        if (room.currentUserIndex >= room.users.length) {
            // LAST TURN: Send to AI
            console.log(`Final prompt for room ${roomId}: ${room.promptInProgress}`);
            room.messages.push({ sender: 'prompt', text: room.promptInProgress });
            
            try {
                const claudeResponse = await anthropic.messages.create({
                    model: "claude-3-haiku-20240307",
                    max_tokens: 1024,
                    messages: [{ role: 'user', content: room.promptInProgress }],
                });
                const aiText = claudeResponse.content[0].text;
                room.messages.push({ sender: 'claude', text: aiText });
            } catch (error) {
                console.error("AI Error:", error);
                room.messages.push({ sender: 'system', text: 'Error connecting to AI.' });
            }
    
            // Reset for the next round
            room.currentUserIndex = 0;
            room.promptInProgress = "";
        }
        
        // Notify everyone of the state change
        io.to(roomId).emit('update_game_state', {
            users: room.users,
            currentUserIndex: room.currentUserIndex,
            promptInProgress: room.promptInProgress,
            messages: room.messages
        });
    });

    // ... disconnect logic would go here ...
});

server.listen(3001, () => console.log('Server is live and listening on port 3001'));